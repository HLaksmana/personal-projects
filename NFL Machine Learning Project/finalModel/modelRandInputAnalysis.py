...
def evaluate_model(model, modelName, train_x, test_x, train_y, test_y):
    ...
    denormTrainPredict = trainPredict * 100
    denormTrainY = train_y * 100
    denormTestPredict = testPredict * 100
...
with open(workDirectory + 'model_rand_input_exp2.csv','a') as f1:
    ...
    #     test_x1, test_x2, test_y1, test_y2 = train_test_split(np.array(xD), np.array(yD), test_size=0.5)
    #     test_x1, test_x3, test_y1, test_y3 = train_test_split(test_x1, test_y1, test_size=0.5)
    #     test_x2, test_x4, test_y2, test_y4 = train_test_split(test_x2, test_y2, test_size=0.5)
        
    #     trainMSE, testMSE = evaluate_model(model, modelPath, test_x1, test_x2, test_y1, test_y2)
    #     writer.writerow([str(x), dropStr, str(trainMSE), str(testMSE)])
    #     trainMSE, testMSE = evaluate_model(model, modelPath, test_x3, test_x4, test_y3, test_y4)
    #     writer.writerow([str(x), dropStr, str(trainMSE), str(testMSE)])
