...
with open('model_Layer_Exp5.csv','w') as f1:
    writer=csv.writer(f1, delimiter=',',lineterminator='\n',)
    for l in LSTM_layers :
        for d in Dense_layers:
            for x in loop:
                # 6-6 nodes
                model, modelPath = create_model(6, 6, l, d)
                trainMSE, testMSE = evaluate_model(model, modelPath)
                writer.writerow([x, 1, l, d, trainMSE, testMSE])
                results.append([x, 1, l, d, trainMSE, testMSE])
                # 4-2 nodes
                model, modelPath = create_model(4, 2, l, d)
                trainMSE, testMSE = evaluate_model(model, modelPath)
                writer.writerow([x, 2, l, d, trainMSE, testMSE])
                results.append([x, 2, l, d, trainMSE, testMSE])
np.savetxt("model_Layer_Exp5backup.csv", results, delimiter=",")
print(results)
