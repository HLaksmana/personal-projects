...
weekPicklePath = "./week_2_4_5_10.pkl"
playPicklePath = "./play_all.pkl"
modPicklePath = "./modPlays_2_4_5_10.pk1"
...
if(pickle):
    ...
    week =  pd.concat([week2, week4, week5, week10], ignore_index=True)
    badGames = [2018092000, 2018102100, 2018101412, 2018110402, 2018102500, 2018110406, 2018111803, 2018112504,2018122307, 2018121500, 2018120600]

    week = week[(week['gameId'] <= gameMax) & (week['playDirection'].str.contains('left')) & (~week['gameId'].isin(badGames))]
...
try:
    ...
    mod = plays[(plays['gameId'] <= gameMax) & (~plays['gameId'].isin(badGames))]
...
def isolatePlay(week, gameNum, playNum):
    ...
    for  player in t.itertuples(index = True):
        ...
        if(player.team_football == 1):
            ...
            if(ps.size == 0):
                print("ps Empty")
                print("gameId", gameNum)
                print("playId", playNum)
                print("frameId", player.frameId)
                print(ps)
                element = np.append(element, [-1, -1])
            else:
...
first = True
matfile = r'E:\All File\partial63varTrainData.mat'
try:
    matdata = scipy.io.loadmat(matfile)
    x = matdata['x']
    y = matdata['y']
    matdata = scipy.io.loadmat('63varTrainData.mat')
    x = np.concatenate(x, matdata['x'], axis = 0)
    y = np.concatenate(y, matdata['y'], axis = 0)
...
def create_model(lstmHiddenNodes, denseNodes, numLSTM, numDense):
    ...
    modelPath = 'E:\\All File\\' + str(numLSTM) + 'x-lstm-hn' + str(lstmHiddenNodes)+'-'+ str(numDense) +'x-dense-'+ str(denseNodes) +'-'+ activation +'-'+ loss +'_'+ str(num_vars) +'Vars_'+ str(batch_size) +'_ep' + str(epochs) +'partial.h5'
    try:
...
model, modelPath = create_model(LSTM_nodes, Dense_nodes, LSTM_layers, Dense_layers)
trainMSE, testMSE = evaluate_model(model, modelPath)

print("Test RMSE:", testMSE)
